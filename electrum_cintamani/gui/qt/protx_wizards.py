...
from electrum_cintamani import cintamani_tx
from electrum_cintamani.bitcoin import COIN, is_b58_address, b58_address_to_hash160
from electrum_cintamani.cintamani_tx import TxOutPoint, service_to_ip_port
from electrum_cintamani.protx import ProTxMN, ProTxService, ProRegTxExc
from electrum_cintamani.util import bfh, bh2u, FILE_OWNER_MODE
...
class ImportLegacyWizardPage(QWizardPage):
    ...
    def update_lmn_data(self, current):
        ...
        if prevout_hash:
            val_cintamani = '%s DASH' % (value/COIN) if value else ''
            self.collateral_val.setText(val_cintamani)
...
class BlsKeysWizardPage(QWizardPage):
    ...
    def initializePage(self):
        ...
        if start_id in parent.UPD_ENTER_PAGES:
            self.setSubTitle(_('Regenerate BLS keypair, setup cintamanid'))
            if not self.bls_priv.text():
                self.bls_priv.setText(new_mn.bls_privk)
                self.bls_pub.setText(new_mn.pubkey_operator)
        else:
            self.setSubTitle(_('Generate BLS keypair, setup cintamanid'))
    ...
    def generate_bls_keypair(self):
        ...
        self.bls_info_label.setText(_('BLS keypair generated. Before '
                                      ...
                                      'line to ~/.cintamanicore/cintamani.conf and '
                                      'restart masternode:'))
        ...
        if self.cb_make_tx.isChecked():
            ...
            try:
                if start_id == parent.OPERATION_TYPE_PAGE:
                    ...
                    tx_descr = 'ProRegTx'
                    tx_type = cintamani_tx.SPEC_PRO_REG_TX
                elif start_id == parent.UPD_SRV_PAGE:
                    pro_tx = manager.prepare_pro_up_srv_tx(self.new_mn)
                    tx_descr = 'ProUpServTx'
                    tx_type = cintamani_tx.SPEC_PRO_UP_SERV_TX
                elif start_id == parent.UPD_REG_PAGE:
                    pro_tx = manager.prepare_pro_up_reg_tx(self.new_mn)
                    tx_descr = 'ProUpRegTx'
                    tx_type = cintamani_tx.SPEC_PRO_UP_REG_TX
            except ProRegTxExc as e:
                gui.show_error(e)
                return True
            gui.do_clear()
            mn = self.new_mn
            if mn.collateral.is_null and tx_type == cintamani_tx.SPEC_PRO_REG_TX:
