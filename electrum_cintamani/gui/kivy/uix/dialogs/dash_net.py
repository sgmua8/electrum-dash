...
from electrum_cintamani import util

from electrum_cintamani.gui.kivy.i18n import _

from electrum_cintamani.gui.kivy.uix.dialogs.question import Question


Builder.load_string('''
#:import _ electrum_cintamani.gui.kivy.i18n._
#:import MIN_PEERS_LIMIT electrum_cintamani.cintamani_net.MIN_PEERS_LIMIT
#:import MAX_PEERS_LIMIT electrum_cintamani.cintamani_net.MAX_PEERS_LIMIT
...
class DashNetDataFlowDialog(Factory.Popup):
    ...
    def update(self):
        res = ''
        peers = self.dn_dlg.cintamani_net.peers
...
class MaxPeersPopup(Factory.Popup):
    ...
    def __init__(self, dn_dlg):
        Factory.Popup.__init__(self)
        self.dn_dlg = dn_dlg
        self.slider.value = dn_dlg.cintamani_net.max_peers

    def dismiss(self, value=None):
        super(MaxPeersPopup, self).dismiss()
        if value is not None:
            self.dn_dlg.cintamani_net.max_peers = value
...
class StaticPeersPopup(Factory.Popup):
    ...
    def __init__(self, dn_dlg):
        Factory.Popup.__init__(self)
        self.dn_dlg = dn_dlg
        self.edit.text = dn_dlg.cintamani_net.cintamani_peers_as_str()

    def dismiss(self, cintamani_peers=None):
        if cintamani_peers is None:
            super(StaticPeersPopup, self).dismiss()
            return

        net = self.dn_dlg.net
        cintamani_net = net.cintamani_net
        res = cintamani_net.cintamani_peers_from_str(cintamani_peers)
        if type(res) == str:
            self.err_label.text = f'Error: {res}'
        else:
            super(StaticPeersPopup, self).dismiss()
            self.dn_dlg.config.set_key('cintamani_peers', res, True)
            net.run_from_another_thread(cintamani_net.set_parameters())
            self.dn_dlg.static_peers = cintamani_net.cintamani_peers_as_str()
...
class BanlistCard(Factory.BoxLayout):
    ...
    def on_remove(self, peer):
        if not peer:
            return
        cintamani_net = self.dn_dlg.cintamani_net
...
class BlsSpeedPopup(Factory.Popup):
    ...
    def __init__(self, dn_dlg):
        Factory.Popup.__init__(self)
        self.cintamani_net = dn_dlg.cintamani_net
        self.clock_e = Clock.schedule_once(self.update, 0.5)

    def update(self, *args, **kwargs):
        start_t = time.time()
        res = self.cintamani_net.test_bls_speed()
...
class DashNetDialog(Factory.Popup):
    ...
    run_cintamani_net = BooleanProperty()
    peers = ListProperty()
    ...
    def __init__(self, app):
        ...
        self.cintamani_net = self.net.cintamani_net
        Factory.Popup.__init__(self)
    ...
    def update(self):
        self.on_cintamani_net_activity()
        self.on_sporks_activity()
        self.on_cintamani_peers_updated()
        self.on_cintamani_banlist_updated()
        self.on_mn_list_diff_updated()
        self.on_network_updated()
        self.run_cintamani_net = self.config.get('run_cintamani_net', True)
        self.max_peers = self.cintamani_net.max_peers
        self.use_static_peers = self.config.get('cintamani_use_static_peers', False)
        self.static_peers = self.cintamani_net.cintamani_peers_as_str()

    def open(self, *args, **kwargs):
        super(DashNetDialog, self).open(*args, **kwargs)
        util.register_callback(self.on_cintamani_net_activity_cb,
                               ['cintamani-net-activity'])
        util.register_callback(self.on_sporks_activity_cb,
                               ['sporks-activity'])
        util.register_callback(self.on_cintamani_peers_updated_cb,
                               ['cintamani-peers-updated'])
        util.register_callback(self.on_cintamani_banlist_updated_cb,
    ...
    def dismiss(self, *args, **kwargs):
        ...
        util.unregister_callback(self.on_cintamani_net_activity_cb)
        util.unregister_callback(self.on_sporks_activity_cb)
        util.unregister_callback(self.on_cintamani_peers_updated_cb)
        util.unregister_callback(self.on_cintamani_banlist_updated_cb)
    ...
    def on_cintamani_net_activity_cb(self, event, *args):
        Clock.schedule_once(lambda dt: self.on_cintamani_net_activity())

    def on_cintamani_net_activity(self):
        read_bytes = self.cintamani_net.read_bytes
        write_bytes = self.cintamani_net.write_bytes
    ...
    def on_sporks_activity(self):
        sporks_dict = self.cintamani_net.sporks.as_dict()
        self.sporks = []
    ...
    def on_cintamani_peers_updated_cb(self, event, *args):
        Clock.schedule_once(lambda dt: self.on_cintamani_peers_updated())

    def on_cintamani_peers_updated(self):
        self.peers = []
        for peer, cintamani_peer in self.cintamani_net.peers.items():
            ua = cintamani_peer.version.user_agent.decode('utf-8')
            self.peers.append((peer, ua))

    def on_cintamani_banlist_updated_cb(self, event, *args):
        Clock.schedule_once(lambda dt: self.on_cintamani_banlist_updated())

    def on_cintamani_banlist_updated(self):
        banlist = self.cintamani_net.banlist
        self.banlist = []
    ...
    def toggle_cintamani_net(self, *args):
        self.run_cintamani_net = not self.config.get('run_cintamani_net', True)
        self.config.set_key('run_cintamani_net', self.run_cintamani_net, True)
        self.net.run_from_another_thread(self.net.cintamani_net.set_parameters())
    ...
    def toggle_use_static_peers(self, *args):
        use_static_peers = not self.config.get('cintamani_use_static_peers', False)
        self.use_static_peers = use_static_peers
        self.config.set_key('cintamani_use_static_peers', use_static_peers, True)
        net = self.net
