...
from . import util
from .cintamani_msg import PRIVATESEND_ENTRY_MAX_SIZE, DSPoolState
from .cintamani_ps_net import (PSMixSession, PRIVATESEND_SESSION_MSG_TIMEOUT,
                          MixSessionTimeout, MixSessionPeerClosed)
from .cintamani_ps_wallet import (PSDataMixin, PSKeystoreMixin, KeyPairsMixin,
                             KPStates, NotFoundInKeypairs, AddPSDataError,
                             SignWithKeypairsFailed)
from .cintamani_ps_util import (PSOptsMixin, PSUtilsMixin, PSGUILogHandler,
...
from .cintamani_tx import PSTxTypes, SPEC_TX_NAMES, CTxIn
...
class PSManager(Logger, PSKeystoreMixin, PSDataMixin, PSOptsMixin,
    ...
    @property
    def state(self):
        '''Current state of PSManager (cintamani_ps_util.PSStates enum)'''
        return self._state
    ...
    def start_mixing(self, password, nowait=True):
        ...
        elif not self.cintamani_net.run_cintamani_net:
        ...
        try:
            ...
            if random.random() > 0.33:
                ...
                while self.state == PSStates.Mixing:
                    dsq = self.cintamani_net.get_recent_dsq(recent_mns)
                    if dsq is not None:
