...
# try to make console log lines short... no timestamp, short levelname, no "electrum_cintamani."
...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_cintamani."):
...
def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("electrum_cintamani_log_*.log")), reverse=True)
    for f in files[keep:]:
        try:
            os.remove(str(f))
        except OSError as e:
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_cintamani_log_{timestamp}_{PID}.log"
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_cintamani."):
        name = name[14:]
...
def configure_logging(config):
    ...
    _logger.info(f"Dash Electrum version: {ELECTRUM_VERSION} - "
                 f"https://electrum.cintamani.org - {GIT_REPO_URL}")
