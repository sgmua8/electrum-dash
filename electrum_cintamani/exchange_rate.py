...
class Poloniex(ExchangeBase):
    async def get_rates(self, ccy):
        ...
        quote_currencies = {}
        cintamani_ticker = json.get('BTC_DASH')
        quote_currencies['BTC'] = Decimal(cintamani_ticker['last'])
...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/cintamani/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/cintamani/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/cintamani/market_chart?vs_currency=%s&days=max' % ccy)
