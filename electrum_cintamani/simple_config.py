...
FEERATE_MAX_RELAY = 10000
FEERATE_STATIC_VALUES = [1000, 1500, 2000, 2500,
...
class SimpleConfig(Logger):
    ...
    def electrum_path(self):
        ...
        self.logger.info(f"electrum-cintamani directory {path}")
        return path
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {'cintamani':8, 'mcintamani':5, 'ucintamani':2, 'duffs':0, 'sat':0}
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-cintamani.conf into user_config[]."""
    if not path:
        return {}
